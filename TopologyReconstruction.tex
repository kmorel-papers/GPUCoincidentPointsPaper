...
\begin{figure*}[!tb]
\subfloat[Marching Cubes]{\includegraphics[height=0.3415\columnwidth]{MarchingCubesOverview.pdf}}
\subfloat[Subdivision]{\includegraphics[height=0.3415\columnwidth]{SubdivisionExample.pdf}}
\subfloat[Face-Centered Tetrahedralization]{\includegraphics[height=0.3415\columnwidth]{TetrahedralExample.pdf}}
\subfloat[Dual Mesh]{\includegraphics[height=0.3415\columnwidth]{DualMeshExample2.pdf}}
...
unique and therefore do not need keys.  For example, the triangles
generated by Marching Cubes are contained within the input cubes and are
known to have no duplicates.  The map operation is designed such that
...
\fix{TODO: Start the next section with a general description of algorithms
  generating cells that need their vertex connections found.  Describe the
  algorithm in terms of Figure~\ref{fig:KeyWeld}, changed to match
  nomenclature of Figure~\ref{fig:ProvenanceFlow}.  Follow that with the
  specifics of the Key-Weld algorithm.  Then discuss actual algorithms
  using this technique.  The following section should describe algorithms
  that output their cells on the reduce.}
