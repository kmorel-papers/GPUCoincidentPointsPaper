...
\title{Provenance-Enabled Topology Construction}
...
\IEEEcompsocthanksitem Kenneth Moreland is with Sandia National Laboratories\protect\\
E-mail: kmorel@sandia.gov}% <-this % stops a space
\thanks{}}
...
\begin{figure*}[!tb]
\centering
\subfloat[Marching Cubes\label{fig:teaser:mc}]{\includegraphics[height=0.32\columnwidth]{MarchingCubesOverview.pdf}}
\quad
\subfloat[Subdivision\label{fig:teaser:subdivision}]{\includegraphics[height=0.32\columnwidth]{SubdivisionExample.pdf}}
\quad
\subfloat[Face-Centered Tetrahedralization\label{fig:teaser:tetrahedral}]{\includegraphics[height=0.32\columnwidth]{TetrahedralExample.pdf}}
\quad
\subfloat[Dual Mesh\label{fig:teaser:dual}]{\includegraphics[height=0.32\columnwidth]{DualMeshExample2.pdf}}
...
The algorithm, hereafter referred to as \proc{Vertex-Weld}, works as
...
to find vertex incidence lists using the \proc{Vertex-Incidence-List} method
...
\begin{codebox}
  \Procname{$\proc{Key-Weld}(\id{keys},\id{values},\id{compare},\id{merge},\id{transform})$\\
    ...
    \id{values}: An array of mergeable elements for each key.\\
...
\begin{figure}[tb]
 \vspace{-0.4cm}
 \begin{codebox}
   \Procname{$\proc{Vertex-Incidence-Lists}(\id{cell-ids}, \\
							\hspace*{4.18cm}\id{cell-connections}, \\
                            \hspace*{4.18cm}\id{counts})$\\
     \id{cell-ids}: An array of cell identifiers for each cell vertex.\\
     \makebox[.2in]{}Usually sequential repeated indices $\{0,0,0,1,1,1,...\}$\\
     \id{cell-connections}: Result from \proc{Key-Weld}.\\
     \id{counts}: Result from \proc{Key-Weld}, cell counts per vertex.\\
   }
   ...
   \zi \Comment The size of each incidence list is stored in \id{count}.
...
Subsequent sections apply the \proc{Key-Weld} algorithm in more
...
\begin{figure}[htb]
\includegraphics[width=0.32\columnwidth]{FaceNoMerge.png}
...
\caption{At left artifacts are visible due to sampling error caused by the
  local operation of our Marching Cubes implementation.  At center surface
  ...
  our coarsening without incurring additional performance costs.}
\label{fig:averaging}
\end{figure}
...
Section~\ref{sec:UsingTopologyProvenance} and the same \proc{Key-Weld}
...
%%\subsection{Incidence list construction}
...
\proc{Vertex-Weld} algorithm, so for comparison we include timings for this
mechanism as well.  Note, however, that the \proc{Vertex-Weld} algorithm
...
\begin{figure}[!tb]
\begin{center}
\includegraphics[width=\linewidth]{supernova.png}
...
over the spatial search in \proc{Vertex-Weld} although the OpenMP
...
previous \proc{Vertex-Weld} technique. From our Marching Cubes and Marching
Tetrahedra implementations, we find that the \proc{Vertex-Weld} approach
...
\proc{Vertex-Weld} approach. Such errors are exceptionally easy to
...
to the output vertices, \proc{Key-Weld} becomes agnostic to such
...
version that uses \proc{Vertex-Weld} to establish new vertex connections.
...
a similar version that uses \proc{Vertex-Weld} to establish new vertex
...
\newblock {Stability of Standing Accretion Shocks, with an Eye toward Core‐Collapse Supernovae}.
...
\bibitem{Moreland_Ayachit_Geveci_Ma_2011}
